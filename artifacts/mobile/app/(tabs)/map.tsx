...
export default function MapScreen() {
  ...
  // ── GPU icon-marker sources: campgrounds, hike POIs, OSM trail dots ─────
  ...
  // One dot per OSM trail at its start point — the start/end flag pair made
  // the map feel cluttered (user request), so OSM trails now match the other
  // trail-dot layers.
  const osmDotsGeo = useMemo(() => {
  ...
  const handleOsmDotPress = useCallback(
    (e: NativeSyntheticEvent<PressEventWithFeatures>) => {
      e.stopPropagation();
      const i = Number(e.nativeEvent.features?.[0]?.properties?.i);
      if (!Number.isFinite(i)) return;
      const f = osmDotsGeo.items[i];
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      >
        ...
        {/* ── OSM trail start dots (GPU, cap 120) ───────────────────────── */}
        {mapStyleLoaded && showOsmOverlay && osmDotsGeo.collection.features.length > 0 && (
          <GeoJSONSource
            id="osm-trail-dots"
            data={osmDotsGeo.collection as never}
            onPress={handleOsmDotPress}
          >
            <Layer
              id="osm-trail-dots-circle"
              type="circle"
              paint={{
                "circle-radius": 7,
                "circle-color": "#3DAA5C",
