...
- `artifacts/mobile/lib/trail-routes.ts` — hardcoded polyline for each of the 8 featured trails (`ca-1`, `ca-3`, `ca-4`, `ca-5`, `ca-6`, `ca-7`, `ca-17`, `ca-20`). `ca-1`/`ca-4`/`ca-6`/`ca-17` are real USFS trail-centerline geometry (via `usfs-api.ts`'s live services); `ca-3`/`ca-5`/`ca-7`/`ca-20` are built from confirmed real landmark/waypoint coordinates (no public GPX exists for those BLM/State Parks areas) rather than surveyed tracks — see in-file comment for provenance per trail.
