...
- `artifacts/mobile/app/(tabs)/map.tsx` — main map screen: trails, navigation, keypoints, Community Notes (48h expiry, Firestore `trails/{trailId}/community_notes`). **Marker rendering (perf):** all high-count point categories (trail dots, NFS/RIDB/NPS/USFS pins, campgrounds, hike POIs, OSM trail dots) render as GeoJSONSource + CircleLayer/SymbolLayer GPU layers — never per-point `<Marker>` views (see `.agents/memory/map-gpu-marker-migration.md`); glyph PNGs live in `assets/map-icons/` (regenerate via `pnpm --filter @workspace/scripts run map-icons:generate`). Native Markers only for low-count/animated items (user pulse, SOS, crew, keypoints, community notes). Also: SOS beacons (`sos_beacons/{uid}` + chat/responders subcollections, unread badge, in-app directions), ride recording, map layers/filters/overlays, offline behavior. Per-mode overlay presets: every activity-mode entry (incl. post-login mount) sets the full overlay stack (offroad = MVUM+OSM; camping = campgrounds+driving markers+OSM; hiking = hiking POIs+NFS lines+hiking markers); LAND OWNERSHIP never preset-touched, NPS cleared outside offroad; manual toggles stick until next mode switch. HIKING POINTS layer (auto-enabled in hiking mode) uses `lib/hiking-pois.ts`; detail sheet SAVE writes Firestore `users/{uid}/hiking_spots` (doc id via `campsiteDocId`); deep-link `focusHikeSpot=1`; POINTS OF INTEREST kind-filter pills live in the hiking filter section.
- `artifacts/mobile/app/(tabs)/garage.tsx` — My Garage: My Rides (Firestore `users/{uid}/vehicles`), Offline Maps (regions + saved trail packs), My Crew (`users/{uid}/crew`), FIND MODS / FIND GEAR AI product search (`/api/mods/search`), Saved Campsites (camping mode, Firestore `users/{uid}/campsites` — saved from the map's campground detail sheet SAVE button; doc id = camp id with "/"→"_"; card tap deep-links to map via `focusLat`/`focusLng`/`focusCampsite=1`; saved sites always render as map markers even outside the 40-mi fetch radius or offline). Offline-region split (user-confirmed): map REGIONS toolbar list = discover/download; Garage shows downloaded regions only (manage/delete + empty state pointing to map). MY TRIPS section (all 3 modes) lists saved AI itineraries (`users/{uid}/itineraries`, cache seed `itineraries:${uid}`) with a detail modal (day-by-day plan, notes, RESERVE links, VIEW ON MAP deep-link, SHARE via crew picker, REMOVE). Sharing writes an immutable snapshot to the recipient's `users/{uid}/shared_trips` (+ a `trip_share` notification); recipient sees a SHARED WITH YOU list (cache seed `sharedtrips:${uid}`) and can SAVE TO MY TRIPS (copies to own itineraries as `shared_{docId}`) or REMOVE.
- `artifacts/mobile/app/(tabs)/assistant.tsx` — AI Trip Assistant chat (SSE streaming); server side in `artifacts/api-server/src/routes/assistant.ts` with tools in `artifacts/api-server/src/lib/tools/`. Rich itinerary cards (dates, destination coords, per-day date/plan/reserveUrl, cell/water/shelter/packing notes — all optional, filled from tool results only; camping tool emits recreation.gov reserveUrl only when RIDB `Reservable===true`); SAVE (gated on persisted messages) writes Firestore `users/{uid}/itineraries/msg_{messageId}` with mode + serverTimestamp, undefined-stripped via JSON round-trip.
- `artifacts/mobile/app/adventure.tsx` — post-login start page: activity pills, AI prompt, one-tap per-mode "plan a trip" suggestion chip (routes to assistant with a canned prompt), animated time-of-day sky. Sets AsyncStorage `adventure.mode`/`adventure.remember`.
