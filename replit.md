...
- `artifacts/mobile/app/(tabs)/garage.tsx` — My Garage: My Rides (Firestore `users/{uid}/vehicles`), Offline Maps (regions + saved trail packs), My Crew (`users/{uid}/crew`), FIND MODS / FIND GEAR AI product search (`/api/mods/search`).
