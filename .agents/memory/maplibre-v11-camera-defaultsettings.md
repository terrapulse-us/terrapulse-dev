...
Never set `center` and `zoom` as direct props on `<Camera>` for the initial/default map position. Use `initialViewState` instead. (`defaultSettings` does NOT exist in this version — it silently breaks the bundle.)

```tsx
// WRONG — fights flyTo and trackUserLocation on every re-render
<Camera ref={cameraRef} center={[-119.4, 36.7]} zoom={7} />

// ALSO WRONG — prop does not exist, causes runtime error
<Camera ref={cameraRef} defaultSettings={{ center: [-119.4, 36.7], zoom: 7 }} />

// CORRECT — only used on first render, never conflicts
<Camera ref={cameraRef} initialViewState={{ center: [-119.4, 36.7], zoom: 7 }} />
```
