...
Spike v2 (Moab full region): **PASSED on device** (July 2026, round 2). Round 1 failed — gray background + trail dots only; every sourced layer blank. Root cause: **pmtiles URLs in style JSON must be fully specified — `pmtiles://file:///abs/path`**. The stripped form `pmtiles:///abs/path` works via runtime addSource (that's why Florence passed) but does NOT resolve when declared in style-JSON sources. Round 2 rendered fully; two follow-ups: (a) mountain areas blurry → DEM was z0-12, overzoom stretch; fixed with z0-13 archive (14→39 MB) + zoom-faded hillshade-exaggeration (0.55@z11 → 0.2@z15); (b) map blank at user's my-location — EXPECTED: archive covers only the Moab bbox and the spike replaces the whole world style; pipeline design must add a low-zoom world-overview base + only force region style when offline/in-region. `artifacts/mobile/lib/region-spike.ts` + LABS "MOAB REGION TEST" toggle in map.tsx. Downloads ~47 MB from the dev api-server (`/api/storage/public-objects/regions/...`): Protomaps vector extract (z0-15, ODbL), terrarium DEM archive (z0-13, public domain AWS elevation tiles), Noto glyphs, sprite v4 light. Builds the full ~70-layer `@protomaps/basemaps` light style client-side and REPLACES `mapStyle` entirely while active.
